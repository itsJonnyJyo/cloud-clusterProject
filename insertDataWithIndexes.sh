...
stations_bucket = client.bucket("stations2")
...
station_rows.each do |row|
  ...
  new_station.indexes['location_bin'] = %w{ station_data[:locationtext] }
...
loopdata_bucket = client.bucket("loopdata2")
...
CSV.foreach("/tmp/freeway_loopdata.csv") do |row|
  ...
  # add secondary indexes
  new_loopdata.indexes['speed_bin'] = %w{ loopdata_data[:speed] }
  new_loopdata.indexes['dectorId_bin'] = %w{ loopdata_data[:detectorid] }
  new_loopdata.indexes['startime_bin'] = %w{ loopdata_data[:starttime] }

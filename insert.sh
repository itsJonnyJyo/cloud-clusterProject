...
station_rows.each do |row|
  ...
  new_station.indexes['location_bin'] = [ station_data[:locationtext] ]
...
CSV.foreach("/tmp/freeway_loopdata.csv") do |row|
  ...
  # add secondary indexes
  new_loopdata.indexes['speed_bin'] = [ loopdata_data[:speed] ]
  new_loopdata.indexes['dectorId_bin'] = [ loopdata_data[:detectorid] ]
  new_loopdata.indexes['startime_bin'] = [ loopdata_data[:starttime] ]
